...
class SequentialBaseModel_Contrastive(BaseModel_Contrastive):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("sequential") as self.sequential_scope:
            ...
            # rnn_outputs : item seq -> user rep, 
            # attention_output_i: item long - > user rep
            
            rnn_outputs, rnn_outputs_user, model_output_u, model_output_i, model_output_u_i, attention_output_u, attention_output_i   = self._build_seq_graph()
            logit_u = self._fcn_net(model_output_u, hparams.layer_sizes, scope="logit_fcnu") 
            # * self._fcn_net(, hparams.layer_sizes, scope="logit_fcnu_c")
            logit_i = self._fcn_net(model_output_i, hparams.layer_sizes, scope="logit_fcni")
            #  * self._fcn_net(model_output_i_c, hparams.layer_sizes, scope="logit_fcni_c")
           ...
           # logitB = self._fcn_net(model_outputB, hparams.layer_sizes, scope="logit_fcnB")
            ...
            #self._add_norm()
            return rnn_outputs, rnn_outputs_user, logit_u, logit_i, logit_u_i, attention_output_i, attention_output_u 
        ...
        for batch_data_input in file_iterator:
           # if self.step == 0:
            ...
            if batch_data_input:
                ...
                (_, _, step_loss, step_data_loss, step_contrasitve_loss, summary) = step_result
                # print(step_loss)
                ...
                if self.step % self.hparams.show_step == 0:
                    print(
                        "self.step {0:d} , total_loss: {1:.6f}, data_loss: {2:.6f}, contrastive_loss: {3:.6f}".format(
                            self.step, step_loss, step_data_loss, step_contrasitve_loss
                        )
                    )
    ...
    def eval(self, sess, feed_dict):
        ...
        return super(SequentialBaseModel_Contrastive, self).eval(sess, feed_dict)
    ...
    def infer(self, sess, feed_dict):
        ...
        feed_dict[self.embedding_keeps] = self.embedding_keep_prob_test
        return super(SequentialBaseModel_Contrastive, self).infer(sess, feed_dict)
    ...
    def _build_embedding(self):
        ...
        with tf.variable_scope("embedding", initializer=self.initializer):
            ...
            self.position_u = tf.get_variable(
                name="position_embedding_u",
                shape=[hparams.max_seq_length, self.item_embedding_dim],
                dtype=tf.float32,
            )
            self.position_i = tf.get_variable(
                name="position_embedding_i",
                shape=[hparams.max_seq_length, self.item_embedding_dim],
                dtype=tf.float32,
            )
    ...
    def _lookup_from_embedding(self):
        ...
        self.position_embedding_u = tf.nn.embedding_lookup(
            self.position_u, tf.tile(tf.expand_dims(tf.range(tf.shape(self.iterator.item_history)[1]), 0), [tf.shape(self.iterator.item_history)[0], 1]), # B*L 
        )
        tf.summary.histogram(
            "position_embedding_output_u", self.position_embedding_u
        )
        self.position_embedding_i = tf.nn.embedding_lookup(
            self.position_i, tf.tile(tf.expand_dims(tf.range(tf.shape(self.iterator.user_history)[1]), 0), [tf.shape(self.iterator.user_history)[0], 1]), # B*L 
        )
        tf.summary.histogram(
            "position_embedding_output_i", self.position_embedding_i
        )
        ...
        self.position_embedding_u = self._dropout(
            self.position_embedding_u, keep_prob=self.embedding_keeps
        )
        self.position_embedding_i = self._dropout(
            self.position_embedding_i, keep_prob=self.embedding_keeps
        )

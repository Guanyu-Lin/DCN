...
class BaseModel:
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        with self.graph.as_default():
            ...
            self.logit_u_i = self._build_graph()

            
            self.predA = self._get_pred(self.logit_u_i, self.hparams.method)
        ...
        if self.hparams.loss == "cross_entropy_loss":
            data_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=tf.reshape(self.logit_u_i, [-1]),
            ...
            logits = tf.reshape(self.logit_u_i, (-1, group))

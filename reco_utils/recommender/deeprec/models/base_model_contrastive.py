...
class BaseModel_Contrastive:
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        with self.graph.as_default():
            ...
            # self.rnn_outputs, self.rnn_outputs_user, self.logit_u, self.logit_i, self.logit_u_i = self._build_graph()
            # rnn_outputs user rep
            self.rnn_outputs, self.rnn_outputs_user, self.logit_u, self.logit_i, self.logit_u_i, self.attention_output_i, self.attention_output_u = self._build_graph()

            self.pred_u = self._get_pred(self.logit_u, self.hparams.method)
            self.pred_i = self._get_pred(self.logit_i, self.hparams.method)
            self.pred_u_i = self._get_pred(self.logit_u_i, self.hparams.method)
            # self.logit = self.logit_u + self.logit_i + self.logit_u_i
            self.logit = self.logit_i
            self.predA = self.pred_i 
    ...
    def _get_lossA(self):
        ...
        self.lossA = tf.add(tf.add(self.data_lossA, self.regular_loss), self.contrastive_loss)
        return self.lossA
    ...
    def _add_summariesA(self):
        tf.summary.scalar("data_lossA", self.data_lossA)
        tf.summary.scalar("regular_loss", self.regular_loss)
        tf.summary.scalar("lossA", self.lossA)
        merged = tf.summary.merge_all()
        return merged

    def _contrastive_loss(self):
        regular_para = self.hparams.contrastive_l2
        ...
        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[0]), tf.nn.l2_loss(self.rnn_outputs - self.target_user_embedding))
        )
        # if self.model_name == "SLIRec":
        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[0]), tf.nn.l2_loss(self.attention_output_i - self.target_user_embedding))
        )
        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[1]), tf.nn.l2_loss(self.attention_output_u - self.target_item_embedding))
        )
        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[1]), tf.nn.l2_loss(self.rnn_outputs_user - self.target_item_embedding))
        )

        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[2]), tf.nn.l2_loss(self.logit_i - self.logit_u_i))
        )
        l2_loss = tf.add(
            l2_loss, tf.multiply(regular_para * int(components[3]), tf.nn.l2_loss(self.logit_u - self.logit_i))
        )

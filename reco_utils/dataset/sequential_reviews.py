...
def data_preprocessing(
    ...
    instance_outputA, instance_outputUA = _create_instance(reviews_outputA, "A")
    _create_item2cate(instance_outputA)

    sampled_instance_fileA = _get_sampled_data(instance_outputA, sample_rate=sample_rate)
    if dataset == 'kuaishou':
        preprocessed_output_A, preprocessed_output_UA = _data_processing_ksA(sampled_instance_fileA, instance_outputUA)

        # preprocessed_output_A = _data_processing_ksA(sampled_instance_fileA)
        _data_generating_ksA(preprocessed_output_A, preprocessed_output_UA, train_fileA, valid_fileA, test_fileA)

    else:
        preprocessed_output_A, preprocessed_output_UA = _data_processing(sampled_instance_fileA, instance_outputUA)
        _data_generating(preprocessed_output_A, preprocessed_output_UA, train_fileA, valid_fileA, test_fileA)
    ...
    _negative_sampling_offline(
        sampled_instance_fileA, valid_fileA, test_fileA, test_file_user, valid_num_ngs, test_num_ngs
    )
...
def _negative_sampling_offline(
    instance_input_file, valid_file, test_file, test_file_user, valid_neg_nums=4, test_neg_nums=49
):
    ...
    for line in valid_lines:
        ...
        while count < valid_neg_nums:
            ...
            if (neg_item) == positive_item or (neg_item) in neg_items or (neg_item) not in u_list:
                continue
            # print()
...
def _data_generating(input_file, input_fileU, train_file, valid_file, test_file, min_sequence=1):
    ...
    for line in f_input:
        ...
        if user_id != last_user_id:
            ...
            if len(dt_str) > 0:
                dt_str = dt_str[:-1]
            if history_clk_num >= min_sequence and len(user_str) > min_sequence:
                fo.write(
                    line_split[1]
                    + "\t"
                    + user_id
...
def _data_generating_ksA(input_file, input_fileU, train_file, valid_file, test_file, min_sequence=1):
    """produce train, valid and test file from processed_output file
    Each user's behavior sequence will be unfolded and produce multiple lines in trian file.
    Like, user's behavior sequence: 12345, and this function will write into train file:
    1, 12, 123, 1234, 12345
...
def group_sequence(test_file, split_length):
    ...
    for line in f_test:
        line_split = line.strip().split("\t")
        item_hist_list = line_split[6].split(",")
...
def _create_instance(reviews_file, domain): # ?
    ...
    for line in f_reviews:
        ...
        if reviews_things[1] not in item_dict:
            item_dict[reviews_things[1]] = []
        user_dict[reviews_things[0]].append((line, float(reviews_things[-1])))
        item_dict[reviews_things[1]].append((line, float(reviews_things[-1])))

        item_list.append(reviews_things[1])
        user_list.append(reviews_things[0])
    ...
    f_reviews.close()
    f_output.close()
    return output_file, output_fileU

def _data_processing_ksA(input_file, input_fileU):
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_outputA")
    output_fileU = os.path.join(dirs, "preprocessed_outputUA")

    f_input = open(input_file, "r")
    f_output = open(output_file, "w")
    f_inputU = open(input_fileU, "r")
    f_outputU = open(output_fileU, "w")
    ...
    f_input.seek(0)
    split_count = 0
    for line, lineU in zip(f_input, f_inputU):
        line = line.strip()
        lineU = lineU.strip()
...
def _data_processing(input_file, input_fileU):
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
    output_fileU = os.path.join(dirs, "preprocessed_outputU")
